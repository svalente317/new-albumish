...
int FtpConnect(const char *host, int port, netbuf **nControl)
{
    ...
    ctrl->buf = GC_MALLOC_ATOMIC(FTPLIB_BUFSIZ);
    if (ctrl->buf == NULL)
    {
        if (ftplib_debug)
            perror("malloc");
...
static int FtpOpenPort(netbuf *nControl, netbuf **nData, int mode, int dir)
{
    ...
    if ((mode == 'A') && ((ctrl->buf = GC_MALLOC_ATOMIC(FTPLIB_BUFSIZ)) == NULL))
    {
        if (ftplib_debug)
            perror("malloc");
        net_close(sData);
...
/*
 * FtpListDirectory - issue an MLSD command and write response to output
 *
 * return 1 if successful, 0 otherwise
 */
int FtpListDirectory(const char *outputfile, const char *path, netbuf *nControl)
{
    return FtpXfer(outputfile, path, nControl, FTPLIB_LIST_DIRECTORY, FTPLIB_ASCII);
}
